...
import { createLoader, parseAsBoolean, parseAsInteger, parseAsJson, parseAsString } from 'nuqs/server'
import { useMemo } from 'react'
import { z } from 'zod/v4'

import { defaultStorefrontFilter, sortSchema, storefrontFilterSchema, type StorefrontFilter } from '@/validation'
...
export const searchParamsParser = {
 ...
 sort: parseAsJson((value) => sortSchema.parse(value)).withDefault([{ field: 'createdAt', direction: 'desc' }]),
 reset: parseAsBoolean.withDefault(false),
 filter: parseAsJson((value) => storefrontFilterSchema.optional().parse(value)).withDefault(defaultStorefrontFilter),
}
